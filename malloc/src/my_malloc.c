...
void merge(struct block *blk)
{
    ...
    while (curr != NULL && curr->free == 1)
    {
        ...
        curr->size += blk->size + align(sizeof(struct block), MALLOC_ALIGN);
        curr->next = blk->next;
    ...
    {
        remove_from_free_list(curr);
        blk->size += curr->size + align(sizeof(struct block), MALLOC_ALIGN);
        blk->next = curr->next;
...
void my_free(void *data)
{
    ...
    {
        //printf("Remove a page(%zu) (blk size: %zu)\n", blk->page_size,
    blk->size); remove_from_free_list(blk); munmap(blk, blk->page_size);
    }*/
}
